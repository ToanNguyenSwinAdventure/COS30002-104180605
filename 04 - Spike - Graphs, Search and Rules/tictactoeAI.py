...
class TicTacToe(object):
    ...
    def __init__(self):
        ...
        self.instruction()
        self.draw_board()
        self.bot = input('Choose your bot || 1 - Minimax AI || 2 - Random Bot || 3 - AI battle || else - PvP : ')
    ...
    def minmax(self, depth, player, alpha=-float('inf'), beta=float('inf')):
        # self.draw_board()
        ...
        if player:
            ...
            for i in range(len(self.board)):
                if self.board[i] == ' ':
                    self.board[i] = 'o'
                    score, _ = self.minmax(depth - 1, False, alpha, beta)
                    self.board[i] = ' '
                    if score > val:
                        val = score
                        move = i
            ...
            for i in range(len(self.board)):
                if self.board[i] == ' ':
                    self.board[i] = 'x'
                    score, _ = self.minmax(depth - 1, True, alpha, beta)
                    self.board[i] = ' '
                    if score < val:
                        val = score
                        move = i
