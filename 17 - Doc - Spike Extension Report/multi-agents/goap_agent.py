...
class GOAP_Agent:
    def __init__(self,plan):
        self.goals = {plan['goal']: plan['status']}
    ...
    def choose_action(self):
        ...
        for key, value in self.actions.items():
            print(f"Best action: {best_action} || Best goal: {best_goal} || Key: {key} || Value: {value}")
            if best_goal in value:
                if best_action is None:
                    ...
                    print(f"Utility: {utility} || Best utility: {best_utility}")
